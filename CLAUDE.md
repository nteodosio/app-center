...
`.freezed.dart`, `.g.dart`, `.mocks.dart`, `.pb*.dart` are committed but ignored by `.claudeignore`. Run `melos generate` after changing models, providers, or `@GenerateNiceMocks` annotations.
